...
<script type="text/javascript">
		function deleteById(ji_id){
			if(window.confirm("是否确定删除")){
				window.location.href = "<%=basePath%>jobInfoAction.do?method=delete&ji_id="+ji_id;
			}
		}
	</script>
</head>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <td valign="top" background="<%=basePath %>pc/images/content-bg.gif"><table width="100%" height="31" border="0" cellpadding="0" cellspacing="0" class="left_topbg" id="table2">
      <tr>
        <td height="31"><div class="titlebt">职位管理</div></td>
      </tr>
    </table></td>
  ...
  <tr>
	...
	<td valign="top" bgcolor="#F7F8F9">
		<table width="90%"  align="center" >
			<tr>
				<td align="right"><A href="<%=basePath %>pc/manage/addJobInfo.jsp">新增</A></td>
			</tr>
		</table>
	  <table width="90%" align="center" border="0" cellpadding="0" cellspacing="0" class="table_bordel0">
		<tr>
          ...
          <td width="17%" nowrap align="center" style="background-image:url(<%=basePath%>images/table0_top.jpg); background-repeat:repeat-x; border:#CDC9C9 solid 1px;"><span class="grat14a"><strong>名称</strong></span></td>
        ...
        <% for(int i = 0 ; i < jobInfoList.size() ; i++){ %>
		 	<tr class="list_a01">
		 	   ...
		 	   <td height="35" align="center" class="table_bordel023" nowrap="true"><span class="grat14a"><%=jobInfoList.get(i).getJi_name() %>&nbsp;</span></td>
		 	   <td height="35" align="center" class="table_bordel023" nowrap="true"><a class="sunti12_blue" href="<%=basePath %>jobInfoAction.do?method=edit&ji_id=<%=jobInfoList.get(i).getJi_id() %>" ><span class="en14">编辑</span></a>&nbsp;<a class="sunti12_blue" href="javascript:deleteById('<%=jobInfoList.get(i).getJi_id() %>')" ><span class="en14">删除</span></a></td>
             </tr>
        <%} %>
      </table>
		           ...
		           <td nowrap="true">
					<div style="text-align:right; width:100%;font-size: 12px" class="st12_HB">
						...
						<%}else if(totalpage==nowpage){ %>
							<a href="<%=basePath %>jobInfoAction.do?method=look&page=1">首页</a> <a href="<%=basePath %>jobInfoAction.do?method=look&page=<%=(nowpage-1) %>">上一页</a> 下一页 末页
						<%}else if(nowpage==1){ %>
							首页 上一页 <a href="<%=basePath %>jobInfoAction.do?method=look&page=<%=(nowpage+1) %>">下一页</a> <a href="<%=basePath %>jobInfoAction.do?method=look&page=<%=totalpage %>">末页</a>
						<%}else{ %>
						<a href="<%=basePath %>jobInfoAction.do?method=look&page=1">首页</a> <a href="<%=basePath %>jobInfoAction.do?method=look&page=<%=(nowpage-1) %>">上一页</a> <a href="<%=basePath %>jobInfoAction.do?method=look&page=<%=(nowpage+1) %>">下一页</a> <a href="<%=basePath %>jobInfoAction.do?method=look&page=<%=totalpage %>">末页</a>
						<%} %>
					</div>
	             </td>
		           </tr>
